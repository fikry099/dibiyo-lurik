...
// Helper format ribuan agar tampilan input rapi
...
export default function PembayaranPOR() {
  ...
  const [statusBayar, setStatusBayar] = useState('dp');
  const [loading, setLoading] = useState(false);
  const [showSuccess, setShowSuccess] = useState(false);
  const [mounted, setMounted] = useState(false);
  
  const [formData, setFormData] = useState({
    nominal: '', metode: 'Cash', diskon: 0, tgl_selesai: '', catatan: ''
  });

  useEffect(() => { setMounted(true); return () => setMounted(false); }, []);

  // Kalkulasi Harga Berdasarkan Item POR
  ...
  // --- AUTO FILL NOMINAL BERDASARKAN STATUS & TOTAL ---
  useEffect(() => {
    if (statusBayar === 'lunas') {
      setFormData(prev => ({ ...prev, nominal: String(Math.round(total)) }));
    } else if (statusBayar === 'dp') {
      setFormData(prev => ({ ...prev, nominal: String(Math.ceil(total * 0.3)) }));
    }
  }, [statusBayar, total]);
  ...
  const handleSubmit = async () => {
    ...
    // Transformasi item sesuai kebutuhan API Pre-Order Reguler
              ...
              <div className="relative flex items-center">
                <input 
                  ...
                  onChange={(e) => {
                    const rawValue = e.target.value.replace(/\D/g, '');
                    if (rawValue === '') {
                      setFormData({ ...formData, diskon: 0 });
                      return;
                    }
                    const num = Number(rawValue);
                    if (num <= 100) {
                      setFormData({ ...formData, diskon: num });
                    }
                  }} 
                />
            ...
            <div className="flex flex-col space-y-1.5 bg-[#5AE3ED1C] border border-[#5AE3ED1C] p-3 rounded-md">
              ...
              <div className="relative w-full flex items-center">
                ...
                <DatePicker
                  ...
                  className="w-full h-[36px] pl-9 pr-3 bg-[#FFE176] border border-[#1A335A] rounded-md text-[#1A335A] font-bold outline-none text-xs placeholder-[#1A335A]/40 cursor-pointer"
                />
              </div>
            </div>
          </div>
