...
// Komponen Skeleton Loader khusus halaman Profil
    ...
    <div className="max-w-4xl mx-auto animate-pulse">
      <div className="p-8 bg-white border shadow-sm border-stone-200 rounded-3xl">
        ...
        <div className="flex justify-center mb-4">
          <div className="rounded-full w-28 h-28 bg-stone-200"></div>
        </div>
        ...
        <div className="mb-8 space-y-4">
          {[...Array(2)].map((_, i) => (
            <div key={i}>
              <div className="w-20 h-4 mb-2 rounded bg-stone-200"></div>
              <div className="w-full h-12 border rounded-lg bg-stone-100 border-stone-200"></div>
            </div>
          ))}
        </div>

        {/* Form Card Skeleton */}
        <div className="p-6 space-y-4 border bg-stone-50 border-stone-200 rounded-2xl">
          <div className="w-32 h-5 mb-2 rounded bg-stone-200"></div>
          <div>
            <div className="w-20 h-4 mb-2 rounded bg-stone-200"></div>
            <div className="w-full h-12 bg-white border rounded-lg border-stone-200"></div>
          </div>
          <div>
            <div className="w-24 h-4 mb-2 rounded bg-stone-200"></div>
            <div className="w-full h-12 bg-white border rounded-lg border-stone-200"></div>
          </div>
          <div className="flex justify-end pt-2">
            <div className="h-10 rounded-lg bg-stone-200 w-28"></div>
          </div>
        </div>
      </div>
    </div>
...
export default function ProfilePage() {
  ...
  const [initialLoading, setInitialLoading] = useState(true) // Loading untuk fetch data pertama kali
  ...
  return (
    <div className="w-full mx-auto space-y-4 text-black font-inter">
      ...
      {/* Bagian Judul disamakan persis dengan halaman sebelumnya */}
      ...
      {/* Tampilan Kondisional: Jika sedang loading awal, tampilkan skeleton */}
      {initialLoading ? (
        <ProfileSkeleton />
      ) : (
