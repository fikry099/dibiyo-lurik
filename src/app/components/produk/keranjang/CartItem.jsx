...
// ====================================================================
// UTILITY: Generator Gradient CSS Lurik (Aman untuk RGB & HEX)
// ====================================================================
...
export default function CartItem({ item, onChange, onRemove }) {
  // 🔥 PERBAIKAN UTAMA: Dukung pengecekan status "COMBO-STUDIO" & item.product?.isCustom
  const isCustomItem = 
    item.isCustom === true || 
  ...
  // Mengambil konfigurasi fleksibel
  ...
  if (isCustomItem && configurasi && configurasi.stripes) {
    ...
    // --- MODE REGULAR: Gunakan Gambar Statis ---
  ...
  return (
    <div className="group bg-[#ffffff] p-3 rounded-xl flex flex-col sm:flex-row items-center gap-4 shadow-md border border-[#2D2219]/10 transition-all hover:border-[#E5BA73]/20">
      
      {/* AREA PRATINJAU VISUAL */}
      <div className="relative flex items-center justify-center w-full h-24 overflow-hidden border rounded-lg shadow-inner sm:w-28 shrink-0 border-[#2D2219]/10 bg-zinc-900">
        
        {visualPreview} 
        
        {/* BADGE DI KIRI ATAS */}
        <div className="absolute top-1.5 left-1.5 z-10 pointer-events-none">
          <span className="text-[8px] font-extrabold tracking-wider uppercase px-1.5 py-0.5 rounded border shadow-sm bg-[#F5F2EB] text-[#E5BA73] border-[#E5BA73]/20">
            {isCustomItem ? (nomorGulungan === "COMBO-STUDIO" ? "COMBO" : "CUSTOM") : `G-${nomorGulungan}`}
          </span>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-2 flex-1 w-full text-xs text-[#2D2219]">
        ...
        <div>
          ...
          <p className="font-semibold text-gray-700">{lebar} cm</p>
        </div>

        {/* Kolom 3: Sisa Stok */}
        <div>
          <p className="text-[9px] text-[#A3A19E] uppercase tracking-wider mb-0.5">Sisa Kain</p>
          <p className={`font-bold ${isCustomItem ? "text-sky-600" : "text-amber-600"}`}>
        ...
        <div>
          ...
          <p className="font-semibold text-gray-700">Rp {hargaPerMeter.toLocaleString('id-ID')}</p>
        </div>
        ...
        <div className="col-span-1">
          ...
          <div className="relative flex items-center max-w-[110px]">
            <input
              ...
              className="w-full px-2 py-1 bg-[#F5F2EB] border border-[#2D2219]/10 rounded-lg text-[#2D2219] font-bold text-center focus:outline-none focus:border-[#E5BA73] focus:ring-1 focus:ring-[#E5BA73] text-xs"
            />
        ...
        <div className="col-span-1 text-right md:text-left md:col-span-1">
          ...
          <p className="font-black text-[#A67D45] text-sm">Rp {totalHarga.toLocaleString('id-ID')}</p>
        </div>

        {/* Tampilan Mobile */}
        <div className="col-span-2 pt-2 mt-1 border-t border-gray-100 md:hidden">
          <p className="text-[9px] text-[#A3A19E] uppercase tracking-wider mb-0.5">Kode Kain</p>
      ...
      <button 
        onClick={onRemove} 
        className="p-2 text-[#A3A19E] hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-colors shrink-0 self-end sm:self-center"
