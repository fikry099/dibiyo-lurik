    ...
    <AnimatePresence>
      ...
      >
        <motion.div 
          ...
          exit="hidden"
          className="bg-[#1A1917] border border-[#E5BA73]/25 w-full max-w-2xl rounded-2xl overflow-hidden shadow-2xl relative p-6 space-y-6 origin-center will-change-transform"
          onClick={(e) => e.stopPropagation()} 
        >
          {/* Tombol Close */}
          <button 
            ...
            className="absolute top-4 right-4 text-[#A3A19E] hover:text-[#E5BA73] transition-colors p-1 disabled:opacity-30"
          >
          ...
          <div className="flex gap-4 p-5 border-b border-[#E5BA73]/10">
            <div className="w-20 h-20 rounded-xl bg-[#252220] border border-[#E5BA73]/15 flex items-center justify-center flex-shrink-0 overflow-hidden">
              {product.gambar_url
                ? <img src={product.gambar_url} alt={productTitle} className="object-cover w-full h-full" />
