...
export default function ListPesananSaya({ pesanan }) {
  ...
  // 1. FILTER UTAMA: Hanya loloskan transaksi yang sudah berhasil dibayar
  const transaksiTerbayar = pesanan.filter((tx) => {
    const s = tx.status_transaksi?.toLowerCase();
    return s === "settlement" || s === "success" || s === "capture";
  });

  // Jika setelah difilter tidak ada transaksi yang sukses berkasnya
  if (transaksiTerbayar.length === 0) {
    return (
      <div className="bg-[#ffffff] border border-dashed border-gray-300 rounded-2xl p-12 text-center">
        <h3 className="text-lg font-serif font-bold text-black">Belum Ada Pesanan yang Dibayar</h3>
        <p className="text-sm text-[#6a5848] mt-2">
          Seluruh riwayat pesanan yang berhasil dicheckout dan dibayar akan muncul di sini.
        </p>
      </div>
    );
  }
  ...
  // ✨ FUNGSI BARU: Tombol Informasi Status Pengerjaan (Diupdate oleh CS)
  const renderStatusPengerjaanButton = (statusPengerjaan, noResi) => {
    const status = statusPengerjaan?.toLowerCase();
    ...
    if (status === "dikirim") {
      return (
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 w-full justify-between bg-blue-50 border border-blue-200 p-3.5 rounded-xl">
          ...
          {noResi ? (
            <span className="inline-block bg-blue-600 text-white font-mono text-[11px] tracking-wider px-3 py-1.5 rounded-lg font-bold shadow-sm">
              RESI: {noResi}
            </span>
          ) : (
            <span className="text-[11px] text-blue-500 italic">Kurir sedang menjemput paket</span>
          )}
        </div>
      );
    }

    // JIKA STATUS: DIPROSES (Atau default jika CS belum set status)
    return (
    ...
    <div className="space-y-6">
      {transaksiTerbayar.map((tx) => (
        ...
        >
          ...
          {/* ✨ CONTAINER BUTTON INFORMASI: Mengambil data status_pengerjaan & no_resi dari database */}
          <div className="mx-6 mt-4">
            {renderStatusPengerjaanButton(tx.status_pengerjaan, tx.no_resi)}
          </div>
