...
export default function Hero() {
  ...
  const slides = [
    {
      ...
      title: (
        <>
          Melestarikan <br />
          Tradisi Melalui <br />
          <span className="italic text-transparent bg-clip-text bg-gradient-to-r from-[#C59B5F] to-[#9E7A44] font-serif font-normal">
            Kreativitas Digital
          </span>
        </>
      ),
  ...
  // --- VARIASI ANIMASI KONTEN (Referensi dari Halaman Login Anda) ---
  ...
  return (
    <section className="relative pt-32 pb-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row items-center justify-between gap-12 min-h-[85vh] group select-none">
      ...
      {/* --- SISI KIRI: DESKRIPSI DINAMIS (BISA RE-ANIMATE TIAP SLIDE BERUBAH) --- */}
      <motion.div
        key={`text-${currentSlide}`} // Kunci utama agar teks beranimasi ulang saat ganti slide
        initial="hidden"
        animate="visible"
        variants={contentContainerVariants}
        className="flex-1 space-y-6 text-center lg:text-left min-h-[380px] flex flex-col justify-center lg:items-start pl-4 lg:pl-12"
      >
        <motion.div
          variants={itemVariants}
          className="inline-flex items-center gap-2 bg-[#C59B5F]/10 border border-[#C59B5F]/20 px-3 py-1 rounded-full text-xs font-medium tracking-wider text-[#A67D45] uppercase self-center lg:self-start"
        >
        ...
        <motion.h1
          variants={itemVariants}
          className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-[#2D2219] leading-[1.15] tracking-tight"
        >
        ...
        <motion.p
          variants={itemVariants}
          className="text-base text-[#6E655C] max-w-xl mx-auto lg:mx-0 leading-relaxed font-light opacity-90"
        >
        ...
        <motion.div
          variants={itemVariants}
          className="flex flex-col justify-center w-full gap-4 pt-2 sm:flex-row lg:justify-start sm:w-auto"
        >
          <Link
            href={slides[currentSlide].ctaPrimary.href}
            onClick={(e) =>
              handleCtaClick(e, slides[currentSlide].ctaPrimary.href)
            }
          ...
          <Link
            ...
            onClick={(e) =>
              handleCtaClick(e, slides[currentSlide].ctaSecondary.href)
            }
        ...
        <motion.div
          variants={itemVariants}
          className="flex justify-center w-full gap-2 pt-6 lg:justify-start"
        >
          {slides.map((_, index) => (
            <button
              key={index}
      ...
      {/* --- SISI KANAN: VISUAL VIDEO KONTEN DINAMIS --- */}
      <div className="flex justify-center flex-1 w-full max-w-lg pr-4 lg:max-w-none lg:justify-end lg:pr-12">
        <motion.div
          ...
          className="relative w-full aspect-[4/5] max-h-[520px] bg-[#EFEBE3] border border-[#2D2219]/5 rounded-[32px] overflow-hidden shadow-2xl flex flex-col justify-between"
        >
          ...
          {/* Area Player Video - Dilengkapi Animasi Fade Per Slide */}
          ...
          <div className="bg-[#F5F2EB] border-t border-[#2D2219]/5 p-4 rounded-b-[32px] space-y-2.5 z-10">
            <div className="flex justify-between text-[10px] tracking-wide text-[#6E655C]">
              ...
              <span className="text-[#C59B5F] font-semibold">
                Atmanegara Lurik Heritage
              </span>
            </div>
