...
export default function CustomizerCanvas({
  ...
  const handleUnduhSemuaBerkasZip = async () => {
    ...
    try {
      ...
      if (fabricBase64)
        folderKustom.file("01_Pratinjau_Serat_Kain.png", fabricBase64, {
          base64: true,
        });

      const shirtBase64 = await captureElement("shirt");
      if (shirtBase64)
        folderKustom.file("02_Mockup_Kemeja_Lurik.png", shirtBase64, {
          base64: true,
        });
      ...
      if (outfitKainBase64)
        folderKustom.file(
          "03_Setelan_Paduan_Kain_Lurik.png",
          outfitKainBase64,
          { base64: true },
        );

      const outfitCelanaBase64 = await captureElement("outfit", "celana");
      if (outfitCelanaBase64)
        folderKustom.file(
          "04_Setelan_Paduan_Celana_Hitam.png",
          outfitCelanaBase64,
          { base64: true },
        );
      ...
      console.error(
        "Gagal memproses pembuatan paket unduhan berkas zip:",
        error,
      );
    } finally {
  ...
  // 💡 KONFIGURASI ANIMASI DATANG DARI KIRI KAKU
  const leftAnimation = {
  ...
  return (
    <motion.div
      ...
      className="w-full lg:w-[58%] flex flex-col justify-between items-center bg-[#F5F2EB] border border-[#E5BA73]/10 rounded-3xl p-6 relative aspect-square lg:aspect-auto"
    >
      ...
      <div className="absolute z-10 flex items-center gap-2 top-7 left-6">
        ...
        <span className="text-[10px] font-bold tracking-widest text-[#E5BA73] uppercase">
          {previewMode === "fabric"
            ? "Pratinjau Tenun Langsung"
            : "Mockup Fitting Aktif"}
        </span>
      </div>
      ...
      <div
        ref={canvasRef}
        className="w-full h-full mt-10 rounded-2xl shadow-inner relative overflow-hidden flex items-center justify-center border border-white/5 bg-[#9e9d9b44]"
      >
        {/* JIKA MODE KAIN */}
        {previewMode === "fabric" && (
          <div
        ...
        {previewMode === "shirt" && (
          <div className="relative w-full h-full max-w-2xl max-h-[500px] flex items-center justify-center">
            ...
            <img
              ...
              alt="Tekstur Baju"
              className="absolute inset-0 object-contain w-full h-full pointer-events-none mix-blend-multiply opacity-70"
            />
          </div>
        )}
                      ...
                      >
                        <div
                          style={{
                            ...patternStyle,
                            maskImage:
                              "url('/mockups/shirt-long-front-mask.png')",
                            WebkitMaskImage:
                              "url('/mockups/shirt-long-front-mask.png')",
                            maskSize: "contain",
                            WebkitMaskSize: "contain",
