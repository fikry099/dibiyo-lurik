      ...
      <div className="space-y-6">
        ...
        <div className="bg-[#ffffff] border border-white/5 rounded-2xl p-4 space-y-4">
          <div className="flex items-center justify-between">
            {/* Kontainer baru untuk menumpuk teks secara vertikal */}
            <div className="flex flex-col gap-1">
              <span className="text-xs font-bold tracking-widest text-[#E5BA73] flex items-center gap-1.5">
                <Palette size={14} /> STRUKTUR BENANG EKSTRAKSI
              </span>
              {/* Teks hapus benang sekarang di bawah dan menggunakan style yang serupa dengan teks info kosong */}
              <span className="text-xs text-gray-500 font-normal ml-5">
                Hapus Benang yang tidak diinginkan
              </span>
            </div>
          ...
          <div className="max-h-[215px] overflow-y-auto pr-1 space-y-2.5 custom-scrollbar">
            {stripes.map((stripe, index) => (
              <div
                key={stripe.id}
                className="flex items-center gap-3 bg-[#cfcfcf] p-2.5 rounded-xl border border-white/5"
              >
                <div className="text-[10px] text-zinc-500 font-mono w-4">
                  #{index + 1}
                </div>
        ...
        <div className="bg-[#E5BA73]/5 border border-[#E5BA73]/10 rounded-xl p-4 flex gap-3">
          ...
          <p className="text-xs text-[#A3A19E] leading-relaxed">
            Warna di atas diekstrak otomatis dari katalog kain referensi. Anda
            dapat memodifikasi ketebalannya secara bebas untuk mendapatkan ritme
            lurik baru.
          </p>
        </div>
      </div>
