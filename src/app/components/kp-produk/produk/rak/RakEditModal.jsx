...
export default function RakEditModal({ isOpen, onClose, onSuccess, rakData }) {
  ...
  // Sinkronisasikan input form ketika data rak dari baris tabel dipilih
  useEffect(() => {
    if (rakData) {
      setNama(rakData.nama || '')
      setErrorMessage('')
    }
  }, [rakData, isOpen])
  ...
  const handleSubmit = async (e) => {
    ...
    const targetId = rakData?.id || rakData?.ID
    if (!targetId) {
      Swal.fire({
        title: 'Error Sistem ❌',
        text: 'ID Lokasi Rak tidak valid atau kosong.',
        icon: 'error',
    ...
    try {
      // Mengirim request PATCH ke route dinamis /api/rak/[id]
      const response = await fetch(`/api/rak/${targetId}`, {
        method: 'PATCH',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(result.message || 'Gagal memperbarui data rak')
      }
      ...
      <div className="bg-white rounded-[20px] shadow-[2px_4px_4px_0px_rgba(0,0,0,0.25)] w-[372px] relative animate-in fade-in zoom-in-95 duration-150">
        ...
        <form onSubmit={handleSubmit} className="px-5 pt-4 pb-5">
          ...
          <input
            ...
            maxLength={100}
            autoFocus
            className="w-full h-[46px] px-3 rounded-[10px] border border-[#1A335A] text-[#000000] text-sm font-medium focus:outline-none focus:ring-1 focus:ring-[#1A335A] transition-all disabled:opacity-60 uppercase"
          ...
          <div className="flex justify-end mt-6">
            <button
              type="submit"
              disabled={isSubmitting || !nama.trim() || nama.trim() === namaAsli}
