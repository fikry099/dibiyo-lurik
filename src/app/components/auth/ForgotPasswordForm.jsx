'use client'

import { useState, useEffect } from 'react'
import Link from 'next/link'
import { useRouter } from 'next/navigation'
import NProgress from 'nprogress' 
import { Lock, Mail, User, Eye, EyeOff, ArrowLeft } from 'lucide-react'
import { motion } from 'framer-motion'

export default function ForgotPasswordForm() {
  const router = useRouter()
  const [role, setRole] = useState('')
  const [email, setEmail] = useState('')
  const [newUsername, setNewUsername] = useState('')
  const [newPassword, setNewPassword] = useState('')
  
  const [showPassword, setShowPassword] = useState(false)
  const [loading, setLoading] = useState(false)
  const [fetchingEmail, setFetchingEmail] = useState(false)
  const [error, setError] = useState('')
  const [success, setSuccess] = useState('')


useEffect(() => {
  // 1. Definisikan fungsi async di dalam useEffect
  const fetchEmail = async () => {
    if (!role) {
      setEmail('');
      return;
    }

    setFetchingEmail(true);
    setError('');

    try {
      const response = await fetch(`/api/auth/forgot-password?role=${role}`);
      const data = await response.json();

      if (response.ok && data.email) {
        setEmail(data.email);
      } else {
        setError(data.message || 'Email tidak ditemukan');
        setEmail('');
      }
    } catch (err) {
      setError('Gagal memuat email');
      setEmail('');
    } finally {
      setFetchingEmail(false);
    }
  };

  // 2. Panggil fungsi tersebut
  fetchEmail();
}, [role]); // Ini sudah benar

  const handleSubmit = async (e) => {
    e.preventDefault()
    setError('')
    setSuccess('')
    setLoading(true)
    NProgress.start()

    try {
      // Menyesuaikan payload dengan struktur POST di Backend
      const res = await fetch('/api/auth/forgot-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          role, 
          newUsername: newUsername.trim() || undefined, 
          newPassword: newPassword || undefined 
        }),
      })

      const resData = await res.json()

      if (!res.ok) {
        throw new Error(resData.message || 'Gagal memperbarui akun')
      }
      
      setSuccess('Akun berhasil diperbarui! Mengalihkan ke halaman login...')
      NProgress.done()
      
      setTimeout(() => {
        router.replace('/auth/login')
      }, 2000)
      
    } catch (err) {
      setError(err.message)
      NProgress.done() 
      setLoading(false)
    }
  }

  const containerVariants = {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: 0.08,
        delayChildren: 0.15
      }
    }
  }

  const itemVariants = {
    hidden: { opacity: 0, y: 15 },
    visible: { 
      opacity: 1, 
      y: 0,
      transition: { type: 'spring', stiffness: 100, damping: 15 }
    }
  }

  return (
    <div className="w-screen h-screen m-0 p-0 flex bg-[#F8F9FA] font-sans select-none items-stretch overflow-hidden">
      
      {/* ================= SISI KIRI: BANNER INFORMASI ================= */}
      <div className="hidden lg:flex w-1/2 bg-gradient-to-br from-[#1A335A] to-[#376DC0] p-16 flex-col justify-between relative overflow-hidden h-full">
        <div className="z-20 pt-16 space-y-10">
          <motion.div 
            initial={{ opacity: 0, x: -50 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.6, type: 'spring', bounce: 0.2 }}
            className="flex items-center gap-4"
          >
            <div className="flex items-center justify-center w-18 h-18 shrink-0">
              <img 
                src="/images/logo.png" 
                alt="Logo Dibyo Lurik" 
                className="object-contain w-full h-full"
                onError={(e) => {
                  e.target.onerror = null;
                  e.target.src = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%23F2B600'%3E%3Cpath d='M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z'/%3E%3C/svg%3E"
                }}
              />
            </div>
            <div>
              <h2 className="text-4xl font-bold tracking-wide text-white">Dibyo Lurik</h2>
              <p className="text-sm font-medium text-white/80">Sistem Manajemen Produk Kain</p>
            </div>
          </motion.div>

          <motion.div 
            initial={{ opacity: 0, x: -40 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.6, delay: 0.15, type: 'spring' }}
            className="max-w-md space-y-4"
          >
            <h1 className="text-3xl font-bold leading-tight tracking-wide text-white">
              Kelola Produk Kain Lurik <br /> 
              dengan <span className="text-[#F2B600] underline decoration-2 underline-offset-4">Mudah</span> & <span className="text-[#F2B600]">Terintegrasi</span>
            </h1>
            <p className="text-sm leading-relaxed text-white/70">
              Sistem manajemen produk kain lurik untuk mengelola stok, pesanan, penjualan, dan laporan secara efisien.
            </p>
          </motion.div>
        </div>

        <motion.div 
          initial={{ opacity: 0, x: -100, y: 50 }}
          animate={{ opacity: 0.95, x: 0, y: 0 }}
          transition={{ duration: 0.8, delay: 0.3, type: 'spring', stiffness: 60 }}
          className="absolute bottom-[40px] left-0 w-64 pointer-events-none z-10"
        >
          <img 
            src="/images/kain-lurik-mockup.png" 
            alt="Gulungan Kain Lurik" 
            className="block object-contain w-full h-auto"
            onError={(e) => { e.target.style.display = 'none' }}
          />
        </motion.div>

        <motion.div 
          initial="hidden"
          animate="visible"
          variants={{
            hidden: { opacity: 0 },
            visible: { opacity: 1, transition: { staggerChildren: 0.1, delayChildren: 0.4 } }
          }}
          className="relative z-20 grid grid-cols-3 gap-4 pt-2 bg-transparent"
        >
          {[
            { bg: '#2900A6', title: 'Kelola Stok', desc: 'Lebih Efisien', svg: <path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/> },
            { bg: '#FF7F7F', title: 'Pantau Pesanan', desc: 'Lebih Mudah', svg: <><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></> },
            { bg: '#4F8A3F', title: 'Laporan Akurat', desc: 'Kapan Saja', svg: <><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></> }
          ].map((item, index) => (
            <motion.div 
              key={index}
              variants={{ hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0 } }}
              className="flex items-center gap-2.5"
            >
              <div className="p-2 text-white shadow-xs rounded-xl shrink-0" style={{ backgroundColor: item.bg }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">{item.svg}</svg>
              </div>
              <p className="text-[11px] font-medium text-white/60 leading-tight">{item.title}<br/><span className="text-white">{item.desc}</span></p>
            </motion.div>
          ))}
        </motion.div>
      </div>

      {/* ================= SISI KANAN: FORM LUPA PASSWORD (PUTIH) ================= */}
      <div className="relative flex flex-col items-center justify-center w-full h-full px-8 overflow-y-auto bg-white lg:w-1/2 sm:px-16 lg:px-24">
        
        <Link 
          href="/auth/login" 
          className="absolute top-8 left-8 lg:left-12 flex items-center gap-2 text-xs font-bold text-gray-500 hover:text-black transition-colors"
        >
          <ArrowLeft size={16} />
          Kembali ke Login
        </Link>

        <motion.div 
          initial="hidden"
          animate="visible"
          variants={containerVariants}
          className="w-full max-w-[420px] flex flex-col py-16"
        >
          
          {success && (
            <div className="flex items-center gap-2 px-4 py-3 mb-4 text-sm font-medium border shadow-xs rounded-xl bg-emerald-50 text-emerald-700 border-emerald-200 animate-fadeIn">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="shrink-0"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><path d="M22 4L12 14.01l-3-3" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
              {success}
            </div>
          )}

          {/* Avatar Header */}
          <motion.div 
            variants={{
              hidden: { opacity: 0, scale: 0.6 },
              visible: { opacity: 1, scale: 1, transition: { type: 'spring', bounce: 0.4, duration: 0.6 } }
            }}
            className="flex flex-col items-center mb-6"
          >
            <div className="w-24 h-24 bg-[#1E4373] text-white rounded-full flex items-center justify-center shadow-xs mb-3">
              <Lock size={44} className="text-white" />
            </div>
            <h3 className="text-xl font-bold tracking-wide text-gray-900">Lupa Password</h3>
            <p className="mt-1 text-xs font-medium text-gray-400 text-center">
              Pilih Peran untuk memuat email otomatis, kemudian perbarui data kredensial Anda.
            </p>
          </motion.div>

          <form onSubmit={handleSubmit} className="space-y-4">
            
            {error && (
              <div className="px-4 py-3 text-sm font-medium text-red-700 border border-red-200 rounded-xl bg-red-50">
                {error}
              </div>
            )}

            {/* Input Select: Peran (Menyesuaikan validRoles backend) */}
            <motion.div variants={itemVariants} className="space-y-1.5">
              <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider">Peran</label>
              <div className="relative">
                <select
                  value={role}
                  onChange={(e) => setRole(e.target.value)}
                  className="w-full h-12 px-5 text-sm text-gray-900 border border-gray-300 rounded-md bg-[#EDF7FC]/50 focus:bg-white transition-all outline-none focus:ring-2 focus:ring-[#2A5C9A]/20 focus:border-[#2A5C9A] font-medium appearance-none cursor-pointer"
                  required
                >
                  <option value="" disabled hidden>Pilih Role / Peran</option>
                  <option value="owner">Owner / Pemilik</option>
                  <option value="kepala_produksi">Kepala Produksi</option>
                  <option value="customer_service">Customer Service (CS)</option>
                </select>
                <div className="absolute pointer-events-none right-5 top-1/2 -translate-y-1/2 text-gray-500">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="m6 9 6 6 6-6"/></svg>
                </div>
              </div>
            </motion.div>

            {/* Input Field: Email Terdaftar (Terisi otomatis lewat GET & dibuat Read-Only) */}
            <motion.div variants={itemVariants} className="space-y-1.5">
              <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider">Email Terdaftar</label>
              <div className="relative">
                <input
                  type="email"
                  value={fetchingEmail ? 'Memuat email terdaftar...' : email}
                  placeholder="Pilih peran terlebih dahulu"
                  className="w-full h-12 px-5 text-sm text-gray-900 border border-gray-300 rounded-md bg-gray-100/80 font-medium placeholder:text-gray-400 cursor-not-allowed select-none outline-none"
                  readOnly
                  required
                />
                <Mail size={16} className={`absolute -translate-y-1/2 right-5 top-1/2 ${fetchingEmail ? 'text-blue-500 animate-pulse' : 'text-gray-400'}`} />
              </div>
            </motion.div>

            {/* Divider */}
            <motion.div variants={itemVariants} className="flex items-center my-2 select-none">
              <div className="flex-grow border-t border-gray-200"></div>
              <span className="flex-shrink mx-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest bg-white px-1">
                Ubah Salah Satu atau Keduanya
              </span>
              <div className="flex-grow border-t border-gray-200"></div>
            </motion.div>

            {/* Input Field: Username Baru */}
            <motion.div variants={itemVariants} className="space-y-1.5">
              <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider">Username Baru (Opsional)</label>
              <div className="relative">
                <input
                  type="text"
                  value={newUsername}
                  onChange={(e) => setNewUsername(e.target.value)}
                  placeholder="Minimal 3 karakter"
                  disabled={!email || fetchingEmail}
                  className="w-full h-12 px-5 text-sm text-gray-900 border border-gray-300 rounded-md bg-[#EDF7FC]/50 focus:bg-white transition-all outline-none focus:ring-2 focus:ring-[#2A5C9A]/20 focus:border-[#2A5C9A] font-medium placeholder:text-gray-400 disabled:opacity-50 disabled:cursor-not-allowed"
                />
                <User size={16} className="absolute text-gray-500 -translate-y-1/2 right-5 top-1/2" />
              </div>
            </motion.div>

            {/* Input Field: Password Baru */}
            <motion.div variants={itemVariants} className="space-y-1.5">
              <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider">Password Baru (Opsional)</label>
              <div className="relative">
                <input
                  type={showPassword ? 'text' : 'password'}
                  value={newPassword}
                  onChange={(e) => setNewPassword(e.target.value)}
                  placeholder="Minimal 6 karakter"
                  disabled={!email || fetchingEmail}
                  className="w-full h-12 px-5 pr-12 text-sm text-gray-900 border border-gray-300 rounded-md bg-[#EDF7FC]/50 focus:bg-white transition-all outline-none focus:ring-2 focus:ring-[#2A5C9A]/20 focus:border-[#2A5C9A] font-medium placeholder:text-gray-400 disabled:opacity-50 disabled:cursor-not-allowed"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  disabled={!email || fetchingEmail}
                  className="absolute text-gray-500 -translate-y-1/2 right-5 top-1/2 hover:text-gray-700 focus:outline-none disabled:opacity-50"
                  tabIndex={-1}
                >
                  {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
                </button>
              </div>
            </motion.div>

            {/* Tombol Submit Utama */}
            <motion.button
              variants={itemVariants}
              whileHover={{ scale: 1.01 }}
              whileTap={{ scale: 0.99 }}
              type="submit"
              disabled={loading || fetchingEmail || (!newUsername && !newPassword)}
              className="w-full h-12 bg-[#F2B600] hover:bg-[#D9A300] text-white font-bold text-sm rounded-md transition-all shadow-md disabled:opacity-50 disabled:cursor-not-allowed mt-3 flex items-center justify-center space-x-2"
            >
              {loading ? (
                <span className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-white animate-spin" viewBox="0 0 24 24" fill="none">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                  </svg>
                  <span>Memproses...</span>
                </span>
              ) : (
                <span>Simpan Perubahan</span>
              )}
            </motion.button>
          </form>

        </motion.div>

        <p className="absolute text-xs font-medium text-center text-gray-400 bottom-6">
          &copy; 2026 Dibyo Lurik. All rights reserved.
        </p>
      </div>

    </div>
  )
}